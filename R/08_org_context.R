# 08_org_context.R v4
#
# Added @noRd to org_contextServer() - internal Shiny module server, not
...
org_context_ui <- function(id) {
  ...
  bslib::layout_columns(
    bslib::card(
      bslib::card_header("Park Units (content) - optional"),
      shiny::helpText("Park units where the DATA were collected. If data span ",
      ...
      shiny::selectInput(ns("content_units"), NULL, choices = NPS_UNIT_CHOICES,
                         multiple = TRUE, width = "100%")
    ),
    bslib::card(
      bslib::card_header("Producing Unit(s)"),
      shiny::helpText("The unit(s) responsible for generating the data package - ",
                      "may be a single park, a network, or several. May overlap ",
                      "with, or differ entirely from, the content units above."),
      shiny::selectInput(ns("producing_units"), NULL, choices = NPS_UNIT_CHOICES,
                         multiple = TRUE, width = "100%")
    ),
    bslib::card(
      ...
      shiny::numericInput(ns("project_id"), "DataStore Project Reference ID",
                          value = NA, width = "100%")
    ),
    bslib::card(
      ...
      bslib::layout_columns(
        shiny::numericInput(ns("new_xref"), NULL, value = NA, width = "100%"),
...
org_contextServer <- function(id) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(input$add_xref, {
      shiny::req(input$new_xref, !is.na(input$new_xref))
      ref <- as.integer(input$new_xref)
      current <- xrefs()
      if (ref %in% current) {
        shiny::showNotification("That reference ID is already in the list.", type = "warning")
      } else {
        xrefs(c(current, ref))
      }
      shiny::updateNumericInput(session, "new_xref", value = NA)
    })
